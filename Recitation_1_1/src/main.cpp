...
void addTeam(string team_name)
{
  // **********************Fill this method
  current_size++;
  Team *new_tarr = new Team[current_size];                  //create a team list
    for (int i = 0; i < current_size-1; i++) 
    {   
        new_tarr[i] = tarr[i];
        /* new_tarr[i].setName(tarr[i].getName());             //copy the existing team list to the new one
        new_tarr[i].setId(tarr[i].getId());*/        
        /*there should be the coppying process of team players*/    
    }

    new_tarr[current_size-1].setName(team_name);            //add the new team
    new_tarr[current_size-1].setId(current_size-1);

    delete [] tarr;                                         //free the old list 
    tarr = new_tarr;                                  //assign to the new list
